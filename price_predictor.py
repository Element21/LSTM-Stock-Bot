...
def quick_tomorrow(
    ...
    print(
        "Last price was {price:.2f} on {date}".format(
            price=fitted_model.df[fitted_model.target_value].values[-1],
            date=fitted_model.df["Date"].values[-1],
        ),
        "Next price is predicted to be {:2f}".format(tomorrows_value),
        sep="\n",
    )
...
class Price_Predictor:
    def __init__(
        ...
        self.result_ticks_freq = round(
            0.25 * len(self.df) * (1 - self.training_to_test_ratio)
        )

        # initialize the model
        self.model = self.model_initialize(self.n_layers)
        if fit_at_start == True:
            self.test_predictions = self.fit_and_test(
                days_forward, self.df, self.split_val
            )
    ...
    def model_initialize(
        ...
        if self.load_model and os.path.isdir(self.path_load):
            ...
            model.add(
                LSTM(units=50, return_sequences=True, input_shape=(self.time_stamps, 1))
            )
    ...
    def fit_and_test(
        ...
        split_val: Optional[int] = None,
    ) -> np.array:  # this method is not meant to be run by the user
        ...
        X_train_scaled = self.scale.fit_transform(
            training_set[self.target_value].values.reshape(-1, 1)
        )
        ...
        to_test = pd.concat((training_set[-self.time_stamps :], test_set))[
            self.target_value
        ].values
    ...
    def plot_data(self, ax=None):
        ...
        self.df.plot(
            ...
            y=[self.target_value],
            ax=ax,
            title=f"{self.code} Price",
            xticks=np.arange(0, len(self.df), self.data_ticks_freq),
        )
        ax.axvline(self.split_val, c="k", linestyle="--")

        xlim = ax.get_xlim()
        ax.text(
            round((self.split_val + xlim[0]) / 2),
            self.df[self.target_value].max() * 0.99,
            s="Training Set",
            horizontalalignment="center",
        )
        ax.text(
            round((self.split_val + xlim[1]) / 2),
            self.df[self.target_value].min(),
            s="Test Set",
            horizontalalignment="center",
        )
    ...
    def predict(self, input_sequence=None, return_info: bool = True) -> float:
        ...
        if return_info:
            print(
                f"In {self.days_forward+1} day(s) the price will be:",
                rescaled.ravel()[0],
            )
...
class Predict_Iterator(Price_Predictor):
    def __init__(
        ...
        training_to_test_ratio = round(
            (0.9 - 0.70) * effort + 0.70, 2
        )  # I want a minimum of 0.70 and a maximum of 0.9
        idx_epoch = (
            0 if effort <= 0.6 else 1 if effort <= 0.75 else 2 if effort <= 0.85 else 3
        )
    ...
    def get_predictions(
        self, days_to_predict: int = 1, predict_from_date: Optional[str] = None
    ) -> list:
        ...
        if predict_from_date is not None:
            ...
            if end_date > start_date:
                ...
                input_sequence = self.df[self.target_value].values[
                    position - self.time_stamps : position
                ]

            else:
                raise ValueError(
                    "The chosen 'predict_from_date' must be antecedent to the end date selected previously!"
                )
        else:
            input_sequence = self.df[self.target_value].values
        ...
        for n in range(1, self.days_to_predict + 1):
            ...
            price_predicted = self.predict(
                input_sequence=input_sequence, return_info=False
            )
